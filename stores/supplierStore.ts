  ...
  actions: {
    async getAllSupplier({ page, pageSize, search, status }: any) {
      ...
      const data: any = await useCustomFetch(
        `/api/v1/suppliers?${page ? `&page=${page}` : ""}${
    ...
    async createSupplier(ValueSupplier: any) {
        ...
        const data: any = await useCustomFetch(`/api/v1/suppliers/create`, {
            method: "POST",
    ...
    async deleteSupplier(id: string) {
        const data: any = await useCustomFetch(`/api/v1/suppliers/delete/${id}`, {
            method: "DELETE",
        });
        return data;
    },
    async updateSupplier({ id, valueSupplier }: any) {
        const data: any = await useCustomFetch(`/api/v1/suppliers/update/${id}`, {
            method: "PUT",
            body: JSON.stringify(valueSupplier),
        });
        return data;
